...
document.addEventListener("DOMContentLoaded", e => {
    ...
    document.querySelector("#tienda").addEventListener("click", function() {
        mostrarTienda();
    });

    async function mostrarTienda() {
        let text = await obtenerTexto("tienda.html");
        main.innerHTML = text;
        mostrarCrearCuenta();

        let botonesRegistro = document.querySelectorAll(".boton-registro");
        for (let i = 0; i < botonesRegistro.length; i++) {
            botonesRegistro[i].addEventListener("click", function() {
                mostrarRegistro()
            })
        }

        let linksJuego = document.querySelectorAll(".link-juego");
        for (let i = 0; i < linksJuego.length; i++) {
            linksJuego[i].addEventListener("click", async function() {
                main.innerHTML = await obtenerTexto("juego.html");
                document.querySelector(".boton-registro").addEventListener("click", mostrarRegistro);
                juego();
            });
        }
    }
    ...
    document.querySelector("#registro").addEventListener("click", mostrarRegistro);

    async function mostrarRegistro() {
        main.innerHTML = await obtenerTexto("registro.html")
        registro();
    }

    document.querySelector("#nosotros").addEventListener("click", async function() {
        main.innerHTML = await obtenerTexto("nosotros.html")
    });



    async function obtenerTexto(link) {
        let text = "";
        try {
    ...
    function mostrarCrearCuenta() {
        ...
        for (let i = 0; i < botones.length; i++) {
            botones[i].addEventListener("click", mostrarLink); 
        }
